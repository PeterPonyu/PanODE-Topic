...
def _load_correlation(ds_name, model_name):
    ...
    # 1. Flat at BIO_RESULTS root: {model}_{dataset}_correlation.npz
    # 2. Model subdirectory:       {model}/{model}_{dataset}_correlation.npz
    # 3. Original nested layout:   {dataset}/{model}/<legacy names>
    named_file = f"{safe_model}_{ds_name}_correlation.npz"
    candidates = [
        BIO_RESULTS / named_file,
        BIO_RESULTS / safe_model / named_file,
    ]
    ...
    for fname in ["latent_gene_correlation.npz", "correlation_matrix.npz",
        ...
        candidates.append(bio_dir / fname)
...
def generate(out_dir):
    ...
    out_path = out_dir / "Fig7_correlation_topic.png"
