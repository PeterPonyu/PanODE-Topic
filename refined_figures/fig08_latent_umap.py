...
def _load_latent_and_umap(ds_name, model_name):
    ...
    latent_fname = f"{safe_model}_{ds_name}_latent_data.npz"

    # Build candidate paths in priority order:
    # 1. model subdirectory  (BIO_RESULTS/Model/Model_dataset_latent_data.npz)
    # 2. flat at root        (BIO_RESULTS/Model_dataset_latent_data.npz)
    # 3. original layout     (BIO_RESULTS/dataset/model/latent.npz etc.)
    candidates = [
        BIO_RESULTS / safe_model / latent_fname,
        BIO_RESULTS / latent_fname,
    ]
    old_bio_dir = BIO_RESULTS / ds_name / safe_model
    for fname in ["latent.npz", "latent_vectors.npz"]:
        candidates.append(old_bio_dir / fname)
...
def generate(out_dir):
    ...
    out_path = out_dir / "Fig8_umap_topic.png"
